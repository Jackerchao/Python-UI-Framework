...
class PySelenium(object):
            ...
            if browser == "firefox" or browser == "ff":
                ...
                capabilities = {'chromeOptions': {'useAutomationExtension': False, \
                                                  'args': ['--no-sandbox', \
                                                           ...
                                                           '--disable-pupup-blocking', \
                                                           '--allow-running-insecure-content', \
                                                           '--disable-extensions' \
                                                           '--disable-infobars']}}
                dr = webdriver.Chrome(desired_capabilities = capabilities, \
                                       executable_path = os.path.join(globalparam.chrome_path, "chromedriver.exe"))
    ...
    def getElesByXpath(self, xpath):
        wait = WebDriverWait(self.driver, 10)
        try:
            return wait.until(EC.visibility_of_all_elements_located((By.XPATH, xpath)))
        except Exception as e:
            return self.driver.find_elemnts_by_xpath(xpath)

    def input(self, ele, val):
        self.driver.input(ele, val)

    def sendKeys(self, ele, value):
        ele.clear()
        ele.send_keys(value)
    ...
    def enterKeys(self, ele):
        ele.send_keys(Keys.ENTER)

    def sendKeysAndEnter(self, ele, val):
        self.sendKeys(ele, val)
        self.enterKeys(ele)

    def screenShot(self):
        # wait(4)
        t1 = time.time()
        picture = time.strftime('%Y%m%d%H%M%S', time.localtime())
        self.driver.get_screenshot_as_file(os.path.join(globalparam.screenshot_path, picture + '.png'))
        self.my_print("Take screenshot: {0}, Spend {1} seconds".format(picture+'.png', time.time() - t1))
        print("screenshot:", picture, ".png")


    def getMenu(self, menu):
        for item in self.getElesByXpath(r"//a[@class = '%s']"):
            if item.text == menu:
                return item
    ...
    def moveOnAndClick(self, ele):
        from selenium.webdriver.common.action_chains import ActionChains
        ActionChains(self.driver).move_to_element(ele).perform()
        ele.click()

    def navigateToSubBra(self, menus):
        from selenium.webdriver.common.action_chains import ActionChains
        ActionChains(self.driver).move_to_element(self.getMenu(menus[0])).perform()
        ActionChains(self.driver).move_to_element(self.getSubMenu(menus[1])).perform()
        if len(menus) == 2 :
            self.getSubMenu(menus[1]).click()
        if len(menus) == 3 :
            self.getSubMenu(menus[2]).click()

    def selectItem(self, val):
        from selenium.webdriver.support.select import Select
        Select(self.getEleByXpath(r"//select[@name = '%s']")).select_by_value(val)

    def scrollTop(self):
        js_0 = "var q=documentElement.scrllTop=0"
        self.driver.execute_script(js_0)

    def scrollBottom(self):
        js = "var q=documentElement.scrllTop=10000"
    ...
    def isElementExist(self, ele):
        try:
            self.driver.find_elements_by_xpath(ele)
            return True
        except:
            return False
